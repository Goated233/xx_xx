...
# -----------------------------
# PROFILE / SEASON / HISTORY / COSMIC HELPERS
# -----------------------------
...
async def winnerlist(ctx):
    ...
    lines = [f"<@{uid}> (`{uid}`)" for uid in ALLOWED_WINNERS]
    embed.description = "\n".join(lines)
...
async def setminaccount(ctx, days: int | None = None):
    ...
    await ctx.send(
        ...
        "Minimum account age requirement cleared."
    )
...
async def setminjoin(ctx, days: int | None = None):
    ...
    save_requirements()
    await ctx.send(
        f"Minimum server join age set to: {days} days"
        if days is not None else
        "Minimum join age requirement cleared."
    )
...
async def setforbidrole(ctx, role: discord.Role | None = None):
    ...
    save_requirements()
    await ctx.send(
        f"Forbidden role set to: {role.mention}"
        if role else
        "Forbidden role cleared."
    )
...
async def gw(ctx, action=None, duration=None, *, prize_and_winners=None):
    ...
    if not users:
        return await ctx.send("No valid users entered the giveaway.")
    ...
    if not winners:
        return await ctx.send("No winners could be selected.")

    current_giveaway["winners"] = [w.id for w in winners]

    winners_lines = [f"{i}. {w.mention}" for i, w in enumerate(winners, start=1)]

    await ctx.send(f"🎉 Winners for **{prize}**:\n" + "\n".join(winners_lines))

    for w in winners:
        try:
            await w.send(f"🎉 You won **{prize}**!")
        except:
            pass
    ...
    record_giveaway_history({
        "prize": prize,
        "winner_ids": [w.id for w in winners],
        "host_id": ctx.author.id,
        "timestamp": datetime.utcnow().isoformat()
    })
    update_host_stats_on_giveaway(ctx.author.id, len(users))
...
async def reroll(ctx):
    ...
    for w in winners:
        try:
            await w.send(f"🎉 You won **{prize}** (reroll)!")
        except:
            pass
...
async def on_reaction_add(reaction, user):
    ...
    await dm_host(current_giveaway["host_id"], content=f"🎉 Entry added: {user.mention}")
...
async def on_reaction_remove(reaction, user):
    ...
    await log_event(embed=embed)
    await dm_host(current_giveaway["host_id"], content=f"❌ Entry removed: {user.mention}")
...
async def profile(ctx, user: discord.User | None = None):
    ...
    embed.add_field(name="Avg Reaction", value=f"{avg:.2f}s" if avg else "N/A", inline=True)
    embed.add_field(name="Fastest Reaction", value=f"{fast:.2f}s" if fast else "N/A", inline=True)
...
class HelpMenu(View):
    ...
    async def admin_cmds(self, interaction: discord.Interaction, button: Button):
        ...
        embed.add_field(name="-setlog <channel_id>", value="Set the log channel.", inline=False)
        embed.add_field(name="-addwinner <user_id>", value="Add a user ID to allowed winners.", inline=False)
        embed.add_field(name="-winnerlist", value="Show all allowed winners.", inline=False)
        embed.add_field(name="-togglehostdm", value="Toggle giveaway DMs.", inline=False)
        embed.add_field(name="-setminaccount / -setminjoin", value="Set account/server age requirements.", inline=False)
        embed.add_field(name="-setreqrole / -setforbidrole", value="Require or forbid a role.", inline=False)
    ...
    async def stats_cmds(self, interaction: discord.Interaction, button: Button):
        ...
        embed.add_field(name="-profile [user]", value="View entrant profile.", inline=False)
        embed.add_field(name="-seasonboard", value="View season leaderboard.", inline=False)
        embed.add_field(name="-hoststats [user]", value="View host stats.", inline=False)
    ...
    async def whitelist_cmds(self, interaction: discord.Interaction, button: Button):
        ...
        embed.add_field(name="-addwl <user>", value="Owner only: add to whitelist.", inline=False)
        embed.add_field(name="-delwl <user>", value="Owner only: remove from whitelist.", inline=False)
